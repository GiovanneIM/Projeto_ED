...
ResultadoBusca BenchmarkBusca::testarBuscaSequencial(const std::vector<int>& vetor, int alvo) {
    ...
    int idx = AlgoritmosBusca::buscaSequencial(vetor, alvo, m);
...
ResultadoBusca BenchmarkBusca::testarBuscaBinaria(const std::vector<int>& vetor, int alvo) {
    ...
    auto inicio = std::chrono::high_resolution_clock::now();
    int idx = AlgoritmosBusca::buscaBinaria(vetor, alvo, m);
    auto fim = std::chrono::high_resolution_clock::now();
...
ResultadoBusca BenchmarkBusca::testarBuscaBinariaInstrumentada(const std::vector<int>& vetor, int alvo) {
    ResultadoBusca resultado;
...
ResultadoBusca BenchmarkBusca::testarBuscaBinariaRecursiva(const std::vector<int>& vetor, int alvo) {
    ResultadoBusca resultado;
    Metricas m;
    
    auto inicio = std::chrono::high_resolution_clock::now();
    int idx = AlgoritmosBusca::buscaBinariaRecursiva(vetor, alvo, m);
    auto fim = std::chrono::high_resolution_clock::now();
    
    resultado.tempo = std::chrono::duration<double>(fim - inicio).count();
    resultado.comparacoes = m.comparacoes;
    resultado.acessos = m.acessos;
    resultado.profundidade = m.profundidadeRecursao;
    resultado.indiceEncontrado = idx;
    
    return resultado;
}

ResultadoBusca BenchmarkBusca::testarBuscaTernaria(const std::vector<int>& vetor, int alvo) {
    ResultadoBusca resultado;
    Metricas m;
    
    auto inicio = std::chrono::high_resolution_clock::now();
    int idx = AlgoritmosBusca::buscaTernaria(vetor, alvo, m);
    auto fim = std::chrono::high_resolution_clock::now();
...
void BenchmarkBusca::executarCenario(const std::string& nomeCenario, std::vector<int>& vetor, int alvo) {
    std::cout << "Executando cenário: " << nomeCenario << " (n=" << vetor.size() << ")" << std::endl;
...
double BenchmarkBusca::calcularDesvioPadrao(const std::vector<ResultadoBusca>& resultados,
                                            double (ResultadoBusca::*membro)() const) {
    if (resultados.size() < 2) return 0.0;
...
void BenchmarkBusca::gerarRelatorio(const std::string& arquivo) {
    ...
    for (int n : config.tamanhos) {
        ...
        if (!resSeq.empty()) {
            out << std::left << std::setw(25) << "Busca Sequencial"
                << std::right << std::fixed << std::setprecision(8)
                << std::setw(15) << calcularMedia(resSeq, &ResultadoBusca::tempo)
                << std::setw(15) << (long long)calcularMedia(resSeq, &ResultadoBusca::comparacoes)
                << std::setw(15) << (long long)calcularMedia(resSeq, &ResultadoBusca::acessos)
                << std::setw(12) << "-" << "\n";
        }
        
        if (!resBin.empty()) {
            out << std::left << std::setw(25) << "Busca Binária"
                << std::right << std::fixed << std::setprecision(8)
                << std::setw(15) << calcularMedia(resBin, &ResultadoBusca::tempo)
                << std::setw(15) << (long long)calcularMedia(resBin, &ResultadoBusca::comparacoes)
                << std::setw(15) << (long long)calcularMedia(resBin, &ResultadoBusca::acessos)
                << std::setw(12) << "-" << "\n";
        }
        
        if (!resBinInst.empty()) {
            out << std::left << std::setw(25) << "Busca Binária Instr."
                << std::right << std::fixed << std::setprecision(8)
                << std::setw(15) << calcularMedia(resBinInst, &ResultadoBusca::tempo)
                << std::setw(15) << (long long)calcularMedia(resBinInst, &ResultadoBusca::comparacoesDetalhadas)
                << std::setw(15) << (long long)calcularMedia(resBinInst, &ResultadoBusca::acessosDetalhados)
                << std::setw(12) << "-" << "\n";
        }
        
        if (!resBinRec.empty()) {
            out << std::left << std::setw(25) << "Busca Binária Recurs."
                << std::right << std::fixed << std::setprecision(8)
                << std::setw(15) << calcularMedia(resBinRec, &ResultadoBusca::tempo)
                << std::setw(15) << (long long)calcularMedia(resBinRec, &ResultadoBusca::comparacoes)
                << std::setw(15) << (long long)calcularMedia(resBinRec, &ResultadoBusca::acessos)
                << std::setw(12) << (long long)calcularMedia(resBinRec, &ResultadoBusca::profundidade) << "\n";
        }
        
        if (!resTer.empty()) {
            out << std::left << std::setw(25) << "Busca Ternária"
                << std::right << std::fixed << std::setprecision(8)
                << std::setw(15) << calcularMedia(resTer, &ResultadoBusca::tempo)
                << std::setw(15) << (long long)calcularMedia(resTer, &ResultadoBusca::comparacoes)
                << std::setw(15) << (long long)calcularMedia(resTer, &ResultadoBusca::acessos)
                << std::setw(12) << "-" << "\n";
        }
