...
struct EstatisticasExecucao {
    ...
    void calcular(const std::vector<T>& dados) {
        ...
        // Média
        T soma = T();
        for (const auto& val : dados) soma += val;
...
struct BenchmarkConfig {
    ...
    std::vector<std::string> algOrdenacao = {
        "BubbleSort",
        "InsertionSort", 
        "SelectionSort",
        "MergeSort",
        "QuickSort",
        "QuickSortRandomizado",
        "HeapSort",
        "ShellSort"
    };
    
    // Algoritmos de busca a serem testados
    std::vector<std::string> algBusca = {
        "BuscaSequencial",
        "BuscaBinaria",
...
private:
    ...
    std::map<std::string, std::map<std::string, std::map<int, std::vector<ResultadoOrdenacao>>>> resultadosOrdenacao;
    std::map<std::string, std::map<std::string, std::map<int, std::vector<ResultadoBuscaCompleto>>>> resultadosBusca;
    
    // Estatísticas computadas
    std::map<std::string, std::map<std::string, std::map<int, EstatisticasExecucao<double>>>> statsTempo;
    std::map<std::string, std::map<std::string, std::map<int, EstatisticasExecucao<long long>>>> statsComparacoes;
    std::map<std::string, std::map<std::string, std::map<int, EstatisticasExecucao<long long>>>> statsAcessos;
    
    // Funções de execução dos algoritmos
    ResultadoOrdenacao executarOrdenacao(const std::string& algoritmo, std::vector<int>& dados);
    ResultadoBuscaCompleto executarBusca(const std::string& algoritmo, const std::vector<int>& dados, int alvo);
    ...
    template<typename T>
    EstatisticasExecucao<T> computarEstatisticas(const std::vector<T>& dados);
    
    template<typename T>
    T computarMediana(std::vector<T> dados);
